...
{{< admonition type=tip title="公式 (4.82) 的推导" open=false >}}
**公式 (4.82) 的推导**（边际化）：

1. **符号与设定**
   -$y \in \mathcal{Y}$：引导变量（如类别、文本）。
   -$q(x_1|y)$：给定$y$时目标$x_1$的条件分布（条件目标分布）。
   -$p_{t|1}(x|x_1)$：与 4.2 节相同，给定终点$x_1$时、$t$时刻的**条件概率路径**（从源到$x_1$的插值密度）。

2. **条件联合分布的假定**
   在给定$Y=y$时，把“$t$时刻状态$X_t$”与“终点$X_1$”的联合密度设为：
  $$
   p_{t,1|Y}(x, x_1|y) = p_{t|1}(x|x_1)\, q(x_1|y).
  $$
   含义：先按$q(\cdot|y)$抽终点$x_1$，再在“给定终点$x_1$”的条件下，$t$时刻的密度为$p_{t|1}(x|x_1)$；即**条件路径只依赖终点$x_1$，不显式依赖$y$**（$y$只通过$q(x_1|y)$影响终点分布）。

3. **对$x_1$边际化得 (4.82)**
   按边际密度定义，给定$y$时$X_t$的密度等于联合对$x_1$积分：
  $$
   p_{t|Y}(x|y) = \int p_{t,1|Y}(x, x_1|y)\, dx_1.
  $$
   代入上面的假定$p_{t,1|Y}(x, x_1|y) = p_{t|1}(x|x_1) q(x_1|y)$，即得
  $$
   p_{t|Y}(x|y) = \int p_{t|1}(x|x_1)\, q(x_1|y)\, dx_1. \quad \text{（即 (4.82)）}
  $$
   因此 **(4.82) 就是在“条件联合 =$p_{t|1}(x|x_1)q(x_1|y)$”的假定下，对$x_1$做边际化得到的引导概率路径**；与无条件情形$p_t(x) = \int p_{t|1}(x|x_1) q(x_1)\, dx_1$的推导完全平行，只是把目标分布从$q(x_1)$换成了条件目标$q(x_1|y)$。所得引导概率路径以引导变量$Y \sim p_Y$为条件，并满足边际端点
{{< /admonition >}}
...
本节将流匹配从欧氏空间$\mathbb{R}^{d}$扩展到一般黎曼流形$M$。通俗地说，黎曼流形是局部行为类似欧氏空间的空间，并配备了广义的距离和角度概念。黎曼流形可用于建模各类数据，例如，地球上自然现象的概率可在球面上建模（Mathieu and Nickel, 2020），而蛋白质骨架通常用矩阵李群来参数化（Jumper et al., 2021）。流在黎曼流形上的扩展归功于Mathieu and Nickel (2020)和Lou et al. (2020)，但他们最初的训练算法需要昂贵的常微分方程（ODE）模拟。遵循Chen and Lipman (2024)的研究，本节中的流匹配解决方案提供了一种可扩展、无模拟的训练算法，用于学习黎曼流形上的生成模型。

### 5.1 黎曼流形
我们考虑具有度量$g$的完整连通光滑黎曼流形$M$。点$x \in M$处的切空间（包含所有在$x$点与$M$相切的向量的向量空间）记为$T_{x}M$。黎曼度量$g$在$T_{x}M$上定义了内积，记为$<u, v>_{g}$，其中$u, v \in T_{x}M$。令$TM = \cup_{x \in M}\{x\} \times T_{x}M$为切丛，它汇集了流形的所有切平面。在后续内容中，定义在切空间上的向量场是构建流形上带速度场的流的重要对象。我们用$U = \{u_{t}\}$表示时变光滑向量场（VFs）的空间，其中$u_{t}: [0,1] \times M \to TM$，且对所有$x \in M$，有$u_{t}(x) \in T_{x}M$。此外，$div_{g}(u_{t})$是关于空间（$x$）参数的黎曼散度。最后，用$dvol_{x}$表示$M$上的体积元，函数$f: M \to \mathbb{R}$在$M$上的积分记为$\int f(x) dvol_{x}$。

### 5.2 流形上的概率、流和速度
流形$M$上的概率密度函数是积分值为1的连续非负函数$p: M \to \mathbb{R}_{≥0}$，即$\int_{M} p(x) dvol_{x} = 1$。我们将概率路径$p_{t}$定义为概率空间$P$中的时变曲线，即$p_{t}: [0,1] \to P$。与时变流类似，$\psi: [0,1] \times M \to M$定义了每个时刻$t$在$M$上的全局微分同胚。

值得注意的是，通过速度场构建基于流的模型自然适用于一般黎曼流形。形式上，重述Mathieu and Nickel (2020)的命题1可得：

**定理8（流的局部存在性和唯一性）** 设$M$是光滑完整流形，$u_{t} \in U$是速度场。若$u$是$C^{\infty}([0,1] \times M, TM)$（特别是局部李普希茨连续），则式(3.19)中的常微分方程存在唯一解，该解是定义在开集$\Omega \supset \{0\} \times M$上的$C^{\infty}(\Omega, M)$微分同胚$\psi_{t}(x)$。

与时变流类似，流常微分方程通常仅在流形上定义局部微分同胚，这意味着对于不同的$x \in M$，$\psi_{t}(x)$可能定义在最大时间区间$[0, t_{x}]$内。与时变流情况相同，我们将使用半开时间区间$t \in [0,1)$，以允许目标分布$q$具有紧支撑（此时$u_{t}$并非处处有定义）。为确保在期望时间区间$[0,1)$内存在解，我们添加可积性约束（见定理2）并再次依赖质量守恒定理。对于具有度量$g$的黎曼流形，黎曼连续性方程为：
$$
\frac{d}{dt} p_{t}(x) + div_{g}(p_{t} u_{t})(x) = 0,
$$
相应的流形质量守恒定理（Villani et al., 2009）表述如下：

**定理9（流形质量守恒）** 设$p_{t}$是概率路径，$u_{t} \in U$是黎曼流形$M$（具有度量$g$）上的局部李普希茨可积向量场。则以下表述等价：
1. 对于$t \in [0,1)$，连续性方程(5.1)成立。
2.$u_{t}$在式(3.24)的意义下生成$p_{t}$。

在上述结果中，$u$可积意味着：
$$
\int_{0}^{1} \int_{\mathcal{M}} \|u_{t}(x)\| p_{t}(x) dvol_{x} dt < \infty.
$$

注意，定理9的假设可导出$M$上的全局微分同胚，进而得到黎曼瞬时变量替换公式：
$$
\frac{d}{dt} log p_{t}(\psi_{t}(x)) = -div_{g}(u_{t})(\psi_{t}(x)).
$$

最后，若对于$X_{0} \sim p$，有$X_{t} = \psi_{t}(X_{0}) \sim p_{t}$，则称$u_{t}$从$p$生成$p_{t}$。

既然流已被证明是流形上有效的生成模型，那么流匹配的原理自然也可迁移到该领域。在黎曼版本的流匹配中，我们旨在找到一个速度场$u_{t}^{\theta} \in U$，生成满足边际约束$p_{0} = p$和$p_{1} = q$的目标概率路径$p_{t}: [0,1] \to P$，其中$p, q$分别表示流形$M$上的源分布和目标分布。由于速度场位于流形的切空间上，黎曼流匹配损失使用在流形各个切平面上定义的布雷格曼散度来比较速度，即：
$$
\mathcal{L}_{RFM}(\theta) = \mathbb{E}_{t, X_{t} \sim p_{t}} D_{X_{t}}(u_{t}(X_{t}), u_{t}^{\theta}(X_{t})). (5.5)
$$

在上述方程中，期望现在是流形上的积分，即对于光滑函数$f: M \to M$和随机变量$X \sim p_{X}$，有$E[f(X)] = \int_{M} f(x) p_{X}(x) dvol_{x}$。对于$x \in M$，布雷格曼散度$D_{x}$可能通过黎曼内积和分配给每个切空间的严格凸函数$\Phi_{x}: T_{x}M \to T_{x}M$定义，即$D_{x}(u, v) := \Phi_{x}(u) - [\Phi_{x}(v) + <u - v, \nabla_{v} \Phi_{x}(v)>_{g}]$。例如，选择黎曼度量$\Phi_{x} = \|\cdot\|_{g}^{2}$，则对于$u, v \in T_{x}M$，有$D_{x}(u, v) = \|u - v\|_{g}^{2}$。

### 5.3 流形上的概率路径
边际概率路径的构建与时变流情况（4.4）类似：
$$
p_{t}(x) = \int_{\mathcal{M}} p_{t}(x | x_{1}) q(x_{1}) dvol_{x_{1}},
$$
其中$p_{t | 1}(x | x_{1})$是流形上定义的条件概率路径。我们还要求满足边界约束：
$$
p_{0} = p, p_{1} = q. (5.7)
$$

例如，可通过要求条件路径$p_{t | 1}(x | x_{1})$满足以下条件来实现这些约束：

$$
p_{0 | 1}(x | x_1) = \pi_{0 | 1}(x | x_1),  p_{1 | 1}(x | x_1) = \delta_{x_1}(x), (5.8)
$$

其中$\pi_{0 | 1}$是条件耦合，$\pi_{0 | 1}(x_{0} | x_{1}) = \pi_{0,1}(x_{0}, x_{1}) / q(x_{1})$。

### 5.4 流形的边际化技巧
边际速度场的边际化技巧（定理3）可直接应用于黎曼情况。考虑条件速度场$u_{t}(x | x_{1}) \in U$，满足：
$u_{t}(\cdot | x_{1})$生成 $p_{t | 1}(\cdot | x_{1}).$(5.9)

则边际速度场$u_{t}(x)$通过对所有目标样本的条件速度场$u_{t}(x | x_{1})$求平均得到：
$$
u_{t}(x) = \int_{\mathcal{M}} u_{t}(x | x_{1}) p_{1 | t}(x_{1} | x) dvol_{x_{1}},
$$

根据概率密度函数的贝叶斯法则，有：
$$
p_{1 | t}(x_{1} | x) = \frac{p_{t | 1}(x | x_{1}) q(x_{1})}{p_{t}(x)}, (5.11)
$$
其定义适用于所有满足$p_{t}(x) > 0$的$x \in M$。

黎曼情况的边际化技巧（定理3）需要对假设1进行如下调整：

**假设2**$p_{t | 1}(x | x_{1})$是$C^{\infty}([0,1) \times M)$，$u_{t}(x | x_{1})$作为$(t, x)$的函数是$C^{\infty}([0,1) \times M, M)$。此外，假设$q$具有紧支撑（即$q(x_{1})$在$\mathbb{R}^{m}$的某个有界集外为0）或$M$是紧的；且对于所有$x \in M$和$t \in [0,1)$，有$p_{t}(x) > 0$。

现在我们可以给出流形边际化技巧定理：

**定理10（流形边际化技巧）** 在假设2下，若$u_{t}(x | x_{1})$是条件可积的且生成条件概率路径$p_{t}(\cdot | x_{1})$，则边际速度场$u_{t}(\cdot)$生成边际概率路径$p_{t}(\cdot)$。

这里的条件可积指的是质量守恒定理（5.2）中可积性条件的条件版本：
$$
\int_{0}^{1} \int_{\mathcal{M}} \int_{\mathcal{M}} \|u_{t}(x | x_{1})\|_{g} p_{t | 1}(x | x_{1}) q(x_{1}) dvol_{x_{1}} dvol_{x} dt < \infty (5.12)
$$
...
黎曼条件流匹配（RCFM）损失为：
$$
\mathcal{L}_{RCFM}(\theta) = \mathbb{E}_{t, X_{1}, X_{t} \sim p_{t | 1}(\cdot | X_{1})} D_{X_{t}}(u_{t}(X_{t} | X_{1}), u_{t}^{\theta}(X_{t})). (5.13)
$$
...
上述定理可通过命题1证明，令$X = X_{t}$、$Y = u_{t}(X_{t} | X_{1})$、$g^{\theta}(x) = u_{t}^{\theta}(x)$，并对$t \in [0,1]$积分即可。
...
其中条件流定义为：
$$
\psi_{t}(x | x_{1}) = \begin{cases} 
x & t = 0 \\
x_{1} & t = 1
\end{cases},
$$
且在$(t, x)$上光滑，在$\mathcal{M}$上关于$x$是微分同胚。（此处的光滑指$\psi_{t}(x | x_{1})$关于$t$和$x$的所有导数都存在且连续，即$C^{\infty}([0,1) \times \mathbb{R}^{d}, \mathbb{R}^{d})$。这些条件可进一步放宽为$C^{2}([0,1) \times \mathbb{R}^{d}, \mathbb{R}^{d})$，但会牺牲一定的简洁性。）

我们在欧氏空间中的分析聚焦于仿射条件流，因为它们是一类易于计算（无模拟）的条件流。但遗憾的是，对于$\alpha_{t} + \sigma_{t} \neq 1$的情况，$\alpha_{t} x_{1} + \sigma_{t} x_{0}$这类组合在流形上并无自然定义。对于$\alpha_{t} + \sigma_{t} = 1$的情况，流形上的类似概念是测地线插值。事实上，Chen and Lipman (2024)提出通过沿测地线构建条件流，特别是将欧氏空间中沿直线移动的条件最优传输（OT）路径推广到流形上（见定理5）。测地线表示流形上两点之间的最短路径，在欧氏空间中退化为直线。对于流形，我们将测地线条件流定义为：
$$
\psi_{t}(x_{0} | x_{1}) = exp_{x_{0}}(\kappa(t) log_{x_{0}}(x_{1})), t \in [0,1], (5.17)
$$
其中$\kappa(t): [0,1] \to [0,1]$是单调递增调度器，满足$\kappa(0) = 0$和$\kappa(1) = 1$，确保所有$x_{0}$在$t = 1$时都能被映射到$x_{1}$。在$x \in M$处计算的指数映射$\exp_{x}: T_{x}M \to M$，$v \mapsto \exp_{x}(v)$，返回从$x$出发、初始速度为$v$的唯一测地线在$t = 1$时的端点。对数映射$log_{x}: M \to T_{x}M$，$y \mapsto log_{x}(y)$是指数映射的逆映射。在欧氏空间中，指数映射就是向量加法，对数映射就是向量减法。将这些代入（5.17），可得$\psi_{t}(x_{0} | x_{1}) = x_{0} + \kappa(t)(x_{1} - x_{0})$，选择$\kappa(t) = t$即可恢复条件最优传输流。

对于具有闭合形式指数映射和对数映射的简单流形，这种构造提供了一种无模拟的流形上流训练方法，与基于流形的扩散模型方法（De Bortoli et al., 2022; Huang et al., 2022b; Lou et al., 2023）相比具有明显优势。特别是，流形扩散模型需要在训练中进行模拟以从$p_{t}$采样，并且必须对流形上的得分函数进行近似。

然而，尽管基于测地线的条件流是一种自然构造，但对于没有闭合形式指数映射和对数映射的一般流形，测地线可能难以计算，并且/或者会引入不期望的偏差（例如概率集中在边界点）。为了克服测地线计算的困难并且/或者注入期望的隐式偏差，可以寻求一种替代的光滑距离函数$d(\cdot, \cdot): M \times M \to \mathbb{R}_{≥0}$，并要求条件流满足：
$$
d(\psi_{t}(x_{0} | x_{1}), x_{1}) = \overline{\kappa}(t) d(x_{0}, x_{1}), (5.18)
$$
其中$\bar{\kappa}(t) = 1 - \kappa(t)$。如果满足以下条件，这将确保条件流在$t = 1$时将所有概率集中在$x_{1}$：
1. 非负性：对所有$x, y \in M$，有$d(x, y) ≥ 0$；
2. 正定性：$d(x, y) = 0$当且仅当$x = y$；
3. 非退化性：$\nabla d(x, y) ≠ 0$当且仅当$x ≠ y$。

Chen and Lipman (2024)表明，满足（5.18）的流对应的最小范数条件速度场形式为：
$$
u_{t}(x | x_{1}) = \frac{d log \overline{\kappa}(t)}{dt} d(x, x_{1}) \frac{\nabla d(x, x_{1})}{\|\nabla d(x, x_{1})\|_{g}^{2}}, (5.19)
$$
其中预度量的非退化性要求确保速度场没有不连续性，因为$u_{t}(x | x_{1}) \propto 1 / \|\nabla d(x, x_{1})\|_{g}$。特别地，（5.17）中的测地线条件流对于选择$d = d_{g}$（其中$d_{g}$是测地线距离）满足（5.18）。替代预度量的一个例子是在一般几何结构上使用谱距离（Chen and Lipman, 2024），其中条件速度提供了一种通过模拟从$p_{t}(x | x_{1})$采样的方法。重要的是，尽管基于预度量的条件流需要在训练中进行模拟（类似于流形上的扩散模型），但与得分函数的近似相比，仍然可以准确地恢复速度场。
...
### 5.7 详细补充文档
  ...
  [Flow Matching Guide and Code 第5章解读：Non-Euclidean Flow Matching](Flow%20Matching%20Guide%20and%20Code%20第5章解读：Non-Euclidean%20Flow%20Matching.md)
...
{{< admonition type=tip title="因子化速度的详细解释" open=false >}}
因子化速度是离散流匹配（DFM）中用来处理高维离散数据（比如句子）的一个核心技巧。

### 核心思想：分工合作，一次只改一个字

想象你要修改一个句子。如果允许你**一次只能修改句子中的一个字**，那么整个句子的变化就可以拆解成每个字独立的变化。

-   **$u_t(y, x)$（整体速度）**：它描述的是整个句子从当前状态$x$变成另一个句子$y$的“冲动”有多强。
-   **因子化速度的假设**：我们认为，只有当$y$和$x$这两个句子**只差一个字**的时候，这个“冲动”才可能是非零的。如果它们有两个或以上的字不同，那“冲动”就是0，因为我们认为这种同时变多个字的情况概率极小，可以忽略。
-   **$u_t^i(y^i, x)$（单字速度）**：既然整个句子的变化可以拆成每个字单独的变化，我们就为每个位置$i$单独训练一个小模型。这个小模型的任务是：**看到当前整个句子$x$，判断把第$i$个位置的字$x^i$改成另一个字$y^i$的“冲动”有多强。**
...
1.  **$\delta(y^{\bar{i}}, x^{\bar{i}})$（判断是否只有一个字不同）**：这是一个“开关”或“检查员”。它的工作是检查句子$y$和句子$x$，除了第$i$个位置之外，其他所有位置的字是不是都一模一样。
    -   如果是（比如$x=$“我爱北京”，$y=$“他爱北京”，只有第一个字不同），那这个“开关”就打开，值为1。
    -   如果不是（比如有两个字不同），那这个“开关”就关闭，值为0。这意味着，对于这个$i$来说，它认为这次整体变化是不可能的，所以贡献为0。

2.  **$u_t^i(y^i, x)$（单字修改的冲动）**：如果“开关”打开了，就轮到第$i$个位置的单字速度模型上场了。它会根据当前整个句子$x$的语境，计算出把第$i$个位置的字从$x^i$改成$y^i$的冲动强度。

3.  **$\sum_i$（求和）**：这个求和是在对所有可能的位置$i$做汇总。但由于“开关”$\delta$的存在，在任何一个具体的$(y, x)$变化中，最多只有一个$i$能让“开关”打开。所以这个求和的结果，要么是0（如果$y$和$x$有两个及以上字不同），要么就是那个唯一不同的位置$i$所对应的单字冲动强度$u_t^i(y^i, x)$。

### 这样做的巨大好处

-   **把不可能变成可能**：如果不做因子化，一个模型需要直接输出所有$K^d$种可能句子的冲动值，这根本做不到。因子化之后，我们只需要训练$d$个小模型（或者一个共享的大模型），每个模型的输出维度是$K$，这就变得完全可行了。
...
{{< admonition type=tip title="因子化速度下的CTMC" open=false >}}

在离散流匹配中，当我们使用**因子化速度**（即假设一次只能改变一个坐标）时，对连续时间马尔可夫链（CTMC）的采样可以简化为每个坐标独立地更新。下面用通俗的语言解释这个推导过程。

---

### 1. 原始设定：微小时间步内的转移概率

CTMC 在很短的时间$h$内，从当前状态$x$转移到新状态$y$的概率可以写成：
$$
\mathbb{P}(X_{t+h}=y \mid X_t=x) = \underbrace{\delta(y,x)}_{\text{留在原地的概率近似为1}} + h \cdot \underbrace{u_t(y,x)}_{\text{转移速率}} + \underbrace{o(h)}_{\text{高阶小量}}.
$$
这个公式的意思是：在极短时间$h$内，要么完全不动（概率 ≈ 1），要么以正比于$h$的概率跳到一个不同的状态，同时还有更小的概率发生多次跳跃（归入$o(h)$）。

---

### 2. 代入因子化速度

因子化速度假设：只有当一个状态$y$与当前状态$x$在**最多一个坐标**上不同时，转移速率才可能非零。具体地，它写成：
$$
u_t(y,x) = \sum_{i=1}^d \underbrace{\delta(y^{\bar{i}}, x^{\bar{i}})}_{\text{检查其他坐标是否相同}} \cdot u_t^i(y^i, x),
$$
其中$\delta(y^{\bar{i}}, x^{\bar{i}})$等于 1 当且仅当$y$和$x$除了第$i$个坐标外完全相同，否则为 0。这意味着$u_t(y,x)$只有在$y$与$x$只差一个坐标时才可能非零，并且此时它等于那个坐标的局部速率$u_t^i(y^i, x)$。

把这个表达式代入第一步的转移概率公式：
$$
\mathbb{P}(X_{t+h}=y \mid X_t=x) = \delta(y,x) + h \sum_i \delta(y^{\bar{i}}, x^{\bar{i}})\, u_t^i(y^i, x) + o(h). \tag{1}
$$
这可以直观理解为：在时间$h$内，要么不变（$\delta$项），要么恰好有一个坐标$i$发生变化，且变化的概率由该坐标的局部速率决定（乘以$h$），其他坐标保持不变。发生两个或更多坐标变化的概率极小，归入$o(h)$。

---

### 3. 写成乘积形式的巧妙之处

注意到$\delta(y,x)$可以写成所有坐标上$\delta$的乘积：
$$
\delta(y,x) = \prod_{i=1}^d \delta(y^i, x^i).
$$
现在考虑每个坐标上的“局部转移概率”：
$$
\delta(y^i, x^i) + h u_t^i(y^i, x).
$$
将它们乘起来：
$$
\prod_{i=1}^d \left[ \delta(y^i, x^i) + h u_t^i(y^i, x) \right].
$$
展开这个乘积：
- 所有因子都取第一项$\delta$，得到$\prod_i \delta(y^i, x^i) = \delta(y,x)$。
- 恰有一个因子（比如第$i$个）取第二项$h u_t^i$，其余取第一项，得到$h u_t^i(y^i, x) \prod_{j\neq i} \delta(y^j, x^j) = h \delta(y^{\bar{i}}, x^{\bar{i}}) u_t^i(y^i, x)$。
- 两个或更多因子取第二项，会得到$h^2$或更高阶项，这些相对于$h$是高阶小量，可以统一归入$o(h)$。

因此，我们有：
$$
\prod_{i=1}^d \left[ \delta(y^i, x^i) + h u_t^i(y^i, x) \right] = \delta(y,x) + h \sum_i \delta(y^{\bar{i}}, x^{\bar{i}}) u_t^i(y^i, x) + o(h). \tag{2}
$$
比较 (1) 和 (2)，右边完全相同，所以：
$$
\mathbb{P}(X_{t+h}=y \mid X_t=x) = \prod_{i=1}^d \left[ \delta(y^i, x^i) + h u_t^i(y^i, x) + o(h) \right]. \tag{3}
$$
...
现在，我们想知道单个坐标$i$在时间$h$后的分布。这可以通过对除了第$i$个坐标外的所有可能值求和得到（即边缘化）：
$$
\mathbb{P}(X_{t+h}^i = y^i \mid X_t=x) = \sum_{y^{\bar{i}}} \mathbb{P}(X_{t+h} = (y^i, y^{\bar{i}}) \mid X_t=x).
$$
代入 (3) 式：
$$
= \sum_{y^{\bar{i}}} \prod_{j=1}^d \left[ \delta(y^j, x^j) + h u_t^j(y^j, x) + o(h) \right].
$$
将乘积拆分为第$i$个因子和其他因子的乘积：
$$
= \left[ \delta(y^i, x^i) + h u_t^i(y^i, x) + o(h) \right] \cdot \sum_{y^{\bar{i}}} \prod_{j\neq i} \left[ \delta(y^j, x^j) + h u_t^j(y^j, x) + o(h) \right].
$$
对每个$j\neq i$，对$y^j$求和：
$$
\sum_{y^j} \left[ \delta(y^j, x^j) + h u_t^j(y^j, x) + o(h) \right] = 1 + h \sum_{y^j} u_t^j(y^j, x) + o(h).
$$
根据速率条件，每个$u_t^j$满足$\sum_{y^j} u_t^j(y^j, x) = 0$（因为留在原地的速率是负的，离开的速率之和为零），所以上式为$1 + o(h)$。因此所有$j\neq i$的乘积求和为$(1+o(h))^{d-1} = 1 + o(h)$。于是：
$$
\mathbb{P}(X_{t+h}^i = y^i \mid X_t=x) = \left[ \delta(y^i, x^i) + h u_t^i(y^i, x) + o(h) \right] \cdot (1+o(h)) = \delta(y^i, x^i) + h u_t^i(y^i, x) + o(h). \tag{4}
$$
这个结果正是我们期望的：每个坐标的演化独立地遵循一个与整体CTMC形式相同的局部转移概率，只是速率函数变成了$u_t^i$。
...
- 想象你有一个由$d$个独立的骰子组成的系统（每个骰子代表一个坐标）。在很短的时间$h$内，每个骰子要么保持原样，要么以微小的概率变成另一个点数，而且不同骰子同时变化的概率极小可以忽略。
- 那么整个系统的状态变化概率，就等于每个骰子独立变化概率的乘积（近似）。
- 当你只关心某个骰子时，只需要看它自己的局部转移概率，而忽略其他骰子，因为它们的可能取值对它的概率没有影响（在$o(h)$精度下）。

因此，因子化速度使得我们可以**按坐标独立采样**：在每个微小时间步，对每个位置，根据其局部速率$u_t^i$，用欧拉方法（如指数欧拉）更新该位置的取值，而无需考虑其他位置的同时变化。这大大简化了采样过程。
